...
## Phase 10 — MCX commodity venue [Decision 037, started 2026-08-29]

Phase 9E waited on a backtest handoff. The handoff arrived and is **MCX natural
gas**, not NSE. User decision: build the venue properly, ship the bucket
disabled, and add a forward dry-run to accumulate the out-of-sample evidence the
run does not have.

**The strategy has NOT cleared this repo's evidence bar and is not recommended
for live money.** Its own handoff says why: no out-of-sample fold (structurally
— TradingView caps intraday history), negative on 30m and 1H over the same
months, and an edge that lives entirely in overnight exposure the exchange stop
cannot protect. The engineering below does not change any of that.

### F1 — Venue + strategy port ✅ 2026-08-29

- [x] `Venue` abstraction in the contract registry — segment, instrument codes
      and Dhan order-segment per exchange. NSE and MCX both load
- [x] **`DerivativeContract.multiplier`** — the field the scrip master does not
      carry. MCX reports `LOT_SIZE` 1 (the order unit) while a Natural Gas Mini
      lot controls 250 mmBtu, which is absent from the file. Order quantity is
      `lots × lot_size`; notional is `lots × multiplier × price`. Identical on
      NSE, 250× apart on MCX
- [x] `MCX_VENUE.require_multiplier` — a contract whose multiplier is unknown is
      REFUSED, not defaulted, because on that venue the default is known-wrong
- [x] Verified against the live master: NATGASMINI notional at ₹171.9 comes to
      **₹42,975**, the figure `trades.json` uses, derived independently
- [x] `TradingType.COMMODITY` + `BucketConfig.exchange` — not
      `Market.COMMODITY`, because 7 of the 10 `Market.INDIAN` sites need the
      INDIAN behaviour for a commodity bucket (sizing cap, margin fit, ledger
      P&L). Rewriting them to `in (INDIAN, COMMODITY)` would have been the
      signal that the distinction was never a market difference
- [x] `src/shared/scanner/cci.py` — pure CCI(20) state machine: two-stage
      arm/fire, intrabar 4.5% stop, ±250 signal exit, arms cleared both sides
      after any exit
- [x] **`scripts/cci_gas_parity.py`: 125/125 exact — 100.0%**, zero missing,
      zero extra, zero side/price/reason mismatches. The strongest parity in
      this repo (swing-indian 208/214, intraday-indian 75/76)
- [x] Parity caught a real port bug: the first cut scored 122/125 and all three
      misses were one rule — **a stop-exit bar still arms**. The stop path
      returned before the arming test ran, so a bar where the stop fired and CCI
      had also gone beyond the arm level never registered the setup
- [x] 18 new tests; **961 green** at the time of commit (956 after the
      unrelated `397b49b` revert removed a 5-test file), ruff clean

**What parity cannot check:** contract selection. Every row's `contract` and
`expiry` are blank because the run used a continuous front-month series and
TradingView publishes no historical roll dates. The handoff refused to back-fill
them rather than hand over a label that cannot be verified — the right call.

### F2 — Remaining before anything can trade

- [ ] MCX session calendar — 09:00–23:30 IST (23:55 in US DST); `nse_session`
      hardcodes 15:30
- [ ] MCX cost card — CTT 0.01% sell-side, NO STT, MCX exchange rate. **The NSE
      card will not reconcile against it and must not be applied to it**
- [ ] A roll / pre-expiry rule. The backtest has none — it never modelled expiry
      at all. Cash settlement removes delivery risk, not expiry
- [ ] `commodity-indian` bucket, its configs, and a `bucket_state` migration
- [ ] The forward dry-run — the whole point of this phase
...
- [x] **PROBE ACCEPTED 2026-09-17** — NSE_EQ + MTF: `POST /v2/forever/orders`
      returned `orderId 32132609163105 PENDING`, cancelled in the same second,
      and the GTT book verified empty afterwards. Decision 035 is available
      for swing-indian. **USER ACTION remaining: `FOREVER_STOPS_ENABLED=true`
      in the VM's `.env` and a restart** — after the token fix below deploys
...
- 2026-09-17 — **Phase 12 deployed; a daily 25-minute blindness found and fixed.** Pushed `0035842`+Phase 12 at 00:23 IST; migration 0015 applied, clean restart 00:28, `NRestarts=0`. The NSE forever-order probe was ACCEPTED (NSE_EQ + MTF, GTT `32132609163105` placed and cancelled, book verified empty), so Decision 035 is live-capable pending the switch. While verifying the GTT book the bot was found mid-outage: `DH-906` on every call from 21:17 IST, "Invalid TOTP" ×3 at 21:20, recovery at 21:23. NOT credentials, NOT the clock (chrony 1.8µs off Google NTP) — the journal shows the shape: the token expired 21:27:06, the 30-minute proactive-refresh margin opened 20:57, and from 20:58 every call re-adopted the SAME expiring token from the shared store (`dhan_token_adopted_peer_before_mint` ×167) instead of minting, until Dhan rejected it at 21:17; the mint then happened under a rate-limit collision and a TOTP burst. **The proactive refresh had never once minted proactively** — yesterday shows the same dance (13 rejections). One guard: a peer token that is itself inside the margin is our own expiring token read back, not a fresh mint — fall through and mint early. 2 tests reproduce the live shape; 1175 green. Committed, NOT pushed: MCX open with a position; deploy after 23:30 or before 09:00 — must be live before tomorrow's window opens at 20:53 IST. Also recorded: the roll re-entry is DECIDED-not-built (user), so the October lot closes 10-13 and nothing reopens November.
