...
- [ ] `win_rate` baselines are BLANK for both buckets — not recorded anywhere
      in this repo. Fill from each `backtest_ref` JSON. ← user-gated
- [ ] Verify swing-indian's PF 2.31 against its backtest_ref JSON; the
      Decision 032 comment's "(train fold)" parenthetical is ambiguous
