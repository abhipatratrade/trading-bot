...
def last_complete_bar_key(now: datetime) -> str | None:
    ...
    # the newest complete bin is the previous calendar day's stub. The caller
    # cross-checks against real bars, so naming a non-trading day is harmless.
    return bar_key(ist_now.date() - timedelta(days=1), _BINS_PER_SESSION - 1)
...
def evaluate(
    ...
    *,
    ...
    ``want_bar_key`` pins which bin must be the signal bar (the scan passes the
    bin it is scanning for). When the symbol's newest complete bin is something
    else — a stale feed, a name that did not trade — the symbol is skipped
    rather than entered on an old signal, mirroring the gap-reversal path's
    "actionable only while it is the most recently closed bar" rule.
    ...
    if want_bar_key is not None and key != want_bar_key:
        return None
...
def mean_touched(
    intraday_bars: list[OHLCVBar], ema_len: int
) -> tuple[bool, Decimal, Decimal] | None:
    """Has the last COMPLETE 1h bin closed back at/above its EMA20?

    Returns ``(touched, close, ema)`` or None when the series is too short.
    This is the strategy's primary exit — evaluated on completed bins only, so
    an in-progress bar that pokes above the mean and falls back never triggers
    a close the backtest would not have taken.
    """
    h1 = resample_1h(intraday_bars)
    if len(h1) < ema_len * 5:
        return None
